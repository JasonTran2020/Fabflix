...
Project 5:
---------
Demo video URL: https://youtu.be/8_HkpJkLGXM
...
* Analysis descriptions are written relative to Single-Instance or Scaled test plan cases. i.e., saying shortest time for single HTTP 1 thread refers to being the shortest compared to other single instance cases
...
|------------------------------------------------|----------------------------------------------|----------------------------|-------------------------------------|---------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Case 1: HTTP/1 thread                          | ![](img/scale-1-http-pooling-graph.PNG)      | 189                        | 33                                  | 18                        | 632 req. All the times (query time, servlet, and JDBC) appear very similar to the http 1 thread pooling single instance. Makes sense, as scaling is meant to help with multiple requests at the same time by splitting the load across different machines. However, with only 1 thread sending 1 request, waiting for the response, and then sending the next request, scaling doesn't help, even if we had 100 machines                                                                             |
| Case 2: HTTP/10 threads                        | ![](img/scale-10-http-pooling-graph.PNG)     | 266                        | 110                                 | 65                        | 4471 req. Explicitly comparing to single instance 10 HTTP pooling, this one runs faster overall, as expected since we have two machines taking care of requests, lightening the load on each compared to making a single machine do all requests. Hence the lower servlet and JDBC time. Also, note that when subtracting out the servlet time from the average query time, the result is similar to the other HTTP cases. Makes sense as scaling shouldn't affect network speed from server to client |
| Case 3: HTTP/10 threads/No connection pooling  | ![](img/scale-10-http-no-pooling-graph.PNG)  | 327                        | 166                                 | 109                       | 3662 req. The lack of pooling makes the servlet, JDBC, and query time slower compared to scaled 10 HTTP pooling as we now open a new connection every time a client makes a request. However, it is still faster than the single instance version of 10 HTTP pooling                                                                                                                                                                                                                                 |
